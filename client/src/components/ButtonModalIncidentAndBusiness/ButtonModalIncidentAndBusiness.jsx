...
const ButtonModalIncidentAndBusiness = ({
  buttonName,
  title,
  spanTitle,
  buttonNameSubmit,
      ...
      <dialog id="my_modal_3" className={`modal`}>
        <div className={`modal-box bg-base-200`}>
          <h3 className="font-libre font-bold py-3 text-xl xl:text-3xl">
            {title} <span className={`text-primary`}>{spanTitle}</span>
          </h3>
          <form method="dialog">
            <button className="btn btn-sm text-primary btn-ghost absolute right-2 top-2">
              ✕
            </button>
            <div className=" space-y-4">
              ...
              <div>
                <label className="form-control w-full">
                  ...
                  <textarea
                    autoComplete="off"
                    className="textarea bg-base-200 border-primary textarea-md w-full resize-none"
                    rows={4}
                  ></textarea>
                </label>
              </div>
