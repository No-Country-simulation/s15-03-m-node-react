...
const ImageCarousel = ({ images }) => {
    ...
    const handlePrev = () => {
        setCurrentIndex((prevIndex) => (prevIndex === 0 ? images.length - visibleCount : prevIndex - 1));
    };

    const handleNext = () => {
        setCurrentIndex((prevIndex) => (prevIndex === images.length - visibleCount ? 0 : prevIndex + 1));
    };
                ...
                <div className="flex space-x-4 overflow-hidden w-full">
                    {visibleImages.map((image, index) => (
                        <img
                            ...
                            src={image}
                            alt={`carousel-${index}`}
                            className={`object-cover m-2 ${visibleCount === 1 ? 'w-full' : visibleCount === 2 ? 'w-1/2' : 'w-1/3'} h-full`}
                        />
                    ))}
                </div>
